...
class loftrInfer(object):
    ...
    def _draw_matchs(self, img1, img2, p1s, p2s, mid_space=10, if_save=False):
        ...
        show[:, :w] = img1.copy()
        show[:, w + 10:] = img2.copy()
        p1s = p1s.astype(np.int32)
        p2s = p2s.astype(np.int32)
    ...
    def run(self, img0_bgr, img1_bgr, lenth=200, use_kmeans=True, if_draw=True, if_save=False, stitch_method=1):
        '''
            ...
            返回值:
                image  拼接图像
        '''
        img0_bgr = cv2.resize(img0_bgr, (640*2, 480))  # 统一尺寸为640x480
        img1_bgr = cv2.resize(img1_bgr, (640*2, 480))
...
if __name__ == "__main__":
    ...
    print(f"img_path_l:{img_path_l}")
    left_img = cv2.imread(img_path_l[0])
    no = 0
    for img1_pth in img_path_l[1:]:
